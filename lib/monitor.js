...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  p.setDebug(true);
// update start
   if(check.pollerParams!=null && check.pollerParams.http_options!=null && check.pollerParams.http_options.method!=null && (check.pollerParams.http_options.method=='POST' || check.pollerParams.http_options.method=='PUT') && check.pollerParams.request_data!=null){
	p.setRequestData(JSON.stringify(check.pollerParams.request_data));
   }
//update end
  p.poll();
};
